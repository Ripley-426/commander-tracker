...
func test_clear_commander_damage_from_source_sets_source_damage_to_zero() -> void:
	...
	assert_true(damage_1.has("p1"))
	assert_true(damage_2.has("p1"))
	assert_eq(int(damage_1.get("p1", -1)), 0)
	assert_eq(int(damage_2.get("p1", -1)), 0)

func test_clear_commander_damage_from_source_keeps_state_valid() -> void:
	var state: Dictionary = GAME_STATE_SCRIPT.create_new_game(3, 40, "p3_side_left")
	var service: Object = _create_service()
	assert_true(service.apply_commander_delta(state, 0, 1, 7))
	assert_true(service.clear_commander_damage_from_source(state, 0))
