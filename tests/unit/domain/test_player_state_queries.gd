...
func test_get_player_life_returns_value_for_valid_index() -> void:
...
func test_get_player_life_returns_zero_for_invalid_index() -> void:
	var state: Dictionary = GAME_STATE_SCRIPT.create_new_game(2, 40, "p2_head_to_head")
...
func test_get_commander_damage_returns_zero_for_missing_source_entry() -> void:
	var state: Dictionary = GAME_STATE_SCRIPT.create_new_game(2, 40, "p2_head_to_head")
	assert_eq(PLAYER_STATE_QUERIES.get_commander_damage(state, 1, 0), 0)

func test_build_commander_rows_for_target_excludes_self() -> void:
...
func test_build_commander_rows_for_target_includes_expected_source_indices() -> void:
	var state: Dictionary = GAME_STATE_SCRIPT.create_new_game(3, 40, "p3_side_left")
	var colors: Array[Color] = [
		Color(1.0, 0.0, 0.0, 1.0),
		Color(0.0, 1.0, 0.0, 1.0),
...
func test_build_commander_rows_for_target_applies_source_colors() -> void:
	var state: Dictionary = GAME_STATE_SCRIPT.create_new_game(3, 40, "p3_side_left")
	var colors: Array[Color] = [
		Color(1.0, 0.0, 0.0, 1.0),
		Color(0.0, 1.0, 0.0, 1.0),
